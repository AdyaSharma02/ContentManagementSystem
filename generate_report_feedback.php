...
<div class="container">
  <h1 style="color: white; margin-top:10px;font-weight: bold;">Feedback based report</h1>
<?php
session_start();
...
if (isset($_POST['start_date']) && isset($_POST['end_date'])) {
    ...
    $query = "SELECT Username, FeedbackText, feedbackDate FROM feedback WHERE feedbackDate BETWEEN ? AND ?";
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<div class="result-message no-feedback">
                <p style=color:gray>No feedbacks found within the selected date range.</p>
              </div>';
    }

    $stmt->close();
} else {
    echo '<div class="result-message no-date">
            <p>Please select start and end dates to generate a report.</p>
          </div>';
}
